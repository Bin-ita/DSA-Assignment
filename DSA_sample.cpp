...
auto measureDuration(const string& label, Func func, Args&&... args)
    -> decltype(func(forward<Args>(args)...)) {
    auto start = high_resolution_clock::now();
    auto result = func(forward<Args>(args)...);
    auto stop = high_resolution_clock::now();
    auto duration = duration_cast<nanoseconds>(stop - start).count();
    cout << label << " took " << duration << " nanoseconds\n";
    return result;
}

// For void-returning functions
template <typename Func, typename... Args>
void measureDurationVoid(const string& label, Func func, Args&&... args) {
    auto start = high_resolution_clock::now();
    func(forward<Args>(args)...);
    auto stop = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(stop - start).count();
    cout << label << " took " << duration << " microseconds\n";
}
    ...
    do {
        ...
        switch (choice) {
            case 1: {
                ...
                cin >> target;
                int index = measureDuration("Binary Search", binarySearch, arr, target);
            ...
            case 2: {
                ...
                cout << "Enter number to search: ";
                cin >> target;
                int index = measureDuration("Exponential Search", exponentialSearch, arr, target);
                cout << (index != -1 ? "Element found at index: " + to_string(index) : "Element not found") << "\n";
                break;
            }
            ...
            case 3: {
                vector<int> arr = original;
                measureDurationVoid("Quick Sort", quickSort, arr, 0, arr.size() - 1);
            ...
            case 4: {
                vector<int> arr = original;
                measureDurationVoid("Merge Sort", mergeSort, arr, 0, arr.size() - 1);
